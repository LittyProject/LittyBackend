...
import {todoSchema} from '../../models/entity/todo';
    ...
    try {
        ...
        if(!server) {
            ...
            if(!userRole[0].perms.find(a=> a.name === 'ADMINISTRATOR').type){
                // @ts-ignore
                if(!userRole[0].perms.find(a=> ['CREATE_TODO_LIST', 'MANAGE_TODO'].includes(a.name)).type){
            ...
            await db.insertEntity(todoData);
            emit(server.id, 'serverChannelEntityCreate', todoData);
