...
export default async function(req: express.Request, res: express.Response) {
    ...
    if(checkOnServer(req.user, req.params.id)) return res.json({message: "success"});

    let srv = await db.getServer(req.params.id);
    if(!srv) return res.notFound;

    if(isBannedOnServer(req.user, req.params.id)) return res.status(404).json({reason: srv.banList.filter(x => x.id == req.user?.id)[0].reason !== "" ? srv.banList.filter(x => x.id == req.user?.id)[0].reason : "You are just banned."});
